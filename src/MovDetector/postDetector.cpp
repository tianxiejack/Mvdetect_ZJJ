...
BOOL  CPostDetect_mv::InitializedMD(int lWidth, int lHeight, int lStride)
{
	...
	if (m_ptemp == NULL)
	{
		m_ptemp = new short[lWidth*lHeight];
		if (m_ptemp == NULL)
		{
			return FALSE;
		}
...
BOOL  CPostDetect_mv::GetMoveDetect(LPBYTE lpBitData,int lWidth, int lHeight, int iStride,  int iscatter/* = 20*/)
{
	...
	memset(m_ptemp, 0, lWidth*lHeight*sizeof(short));//全部置成0
	...
	for(t=1; t<=patternnum; t++)//记录每个样品（独立连通区域）的左上、右下点坐标
	{
		for(j=BOL; j<lHeight-BOL; j++)//搜索整幅图像
		{
			...
			{
				if(*(m_ptemp + j*lWidth + i) == (short)m_iRelative[t])
				{
